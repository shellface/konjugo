...
export function buildChallenge(spec: ChallengeSpec): GrammarChallenge {
  ...
  const usesTrigger = cueMode === 'L2' || (cueMode === 'context' && c !== 'nom');
  if (usesTrigger) {
    const trig = spec.trigger;
    if (!trig) throw new Error(`${cueMode} challenge requires a trigger`);
    if (trig.case !== c) {
      throw new Error(
  ...
  // cueMode === 'context' && c === 'nom' → no cue token at all.
...
export function pickSpec(rng: Rng, c: SpecConstraints): ChallengeSpec {
  ...
  if (c.cueMode === 'L2' || (c.cueMode === 'context' && chosenCase !== 'nom')) {
